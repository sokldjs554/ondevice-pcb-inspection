...
| SPI / I2C로 판정 내보내기 | X | O | `--spi`, `--i2c` (리눅스 ioctl), 바이너리 프레임 + CRC-16 |
...
src/serial_bus.*      SPI/I2C 전송, 바이너리 프레임 + CRC-16, 하드웨어 없이 도는 자체 점검
...
# 보드 내부 버스(SPI/I2C) 프레임 자체 점검 — 하드웨어 없이 확인 가능
./deploy/cpp/build/detect_board --bus-selftest
```
...
보드 20장을 두 장비에서 각각 재봤습니다.

| 장비 | 추론 스레드 | 순차 | 파이프라인 | 결과 |
|---|---|---|---|---|
| 4코어 리눅스 | 4 (코어 전부) | 3.62초 | 3.90초 | **8% 느림** |
| 4코어 리눅스 | 1 | 7.57초 | 6.83초 | **10% 빠름** |
| 2코어 노트북(macOS) | 기본(코어 전부) | 9.42초 | 9.73초 | **3% 느림** |
| 2코어 노트북(macOS) | 1 | 13.33초 | 13.02초 | **2% 빠름** |

두 장비에서 방향이 같았습니다. 추론이 이미 코어를 전부 쓰고 있으면 생산자 스레드가
끼어들면서 서로 CPU를 뺏어 손해입니다. 반대로 추론 스레드를 1개로 묶어 코어를 남겨두면
읽기·전처리가 그 여유 코어에서 돌아 추론과 진짜로 겹칩니다.
...
## SPI / I2C로 판정 내보내기 (`--spi`, `--i2c`, `--bus-selftest`)

TCP는 관리 PC로 결과를 모을 때 쓰지만, 검사 장비 안에서 판정을 바로 받아야 하는 쪽은
보통 MCU나 PLC입니다. 그 경로는 Ethernet이 아니라 **SPI나 I2C 같은 보드 내부 버스**입니다.
이 버스는 대역폭이 낮아 JSON을 그대로 흘리기 어려워서, 길이가 정해진 바이너리 프레임을
설계했습니다.
...
오프셋  크기  내용
  ...
  3      1   보드 판정 (0=정상, 1=불량)
  4      2   payload 길이
  6      N   payload = [보드 번호 4][결함 수 1] + 결함마다 10바이트
 6+N     2   CRC-16/CCITT-FALSE (헤더+payload 전체)
```

결함 하나가 10바이트(종류 1 + 좌표 8 + 확신도 1)라 6건이면 총 73바이트입니다.
확신도는 0~1 실수를 1바이트로 줄였는데, 0.4% 해상도면 판정용으로 충분하다고 봤습니다.
결함이 아무리 많아도 한 프레임이 커지지 않도록 24건에서 자릅니다(수신 쪽 버퍼 보호).

구조는 **프레임 만들기/해석하기**와 **실제로 내보내는 통로**를 분리했습니다.

| 통로 | 구현 |
|---|---|
| SPI | `open(/dev/spidevX.Y)` + `ioctl(SPI_IOC_WR_MODE/BITS/SPEED)` + `SPI_IOC_MESSAGE` |
| I2C | `open(/dev/i2c-N)` + `ioctl(I2C_SLAVE, addr)` + `write()` |
| 루프백 | 보낸 바이트를 그대로 돌려줌 (하드웨어 없이 검증용) |

### 하드웨어 없이 어디까지 확인했나

SPI/I2C 장치가 없어서 **루프백 통로로 프레이밍과 오류 검출까지** 확인했습니다.
...
```
프레임 43바이트 (결함 3건) — 헤더 6 + payload 35 + CRC 2
앞 12바이트: A5 5A 01 01 23 00 28 A0 00 00 03 01
왕복 확인: 보드 번호 41000, 결함 3건 → 원본과 일치
1비트 손상 검출: 잡아냄 (CRC가 맞지 않습니다)
잘린 프레임 검출: 잡아냄 (길이가 맞지 않습니다)

통과. SPI 1MHz 기준 43바이트는 약 0.34 ms, 100kHz I2C에서는 약 3.9 ms 걸립니다.
```

즉 **프레임 설계·직렬화·CRC·오류 처리는 검증했고, 실제 버스 파형은 확인하지 못했습니다.**
spidev/i2c-dev 코드는 리눅스 ioctl API를 그대로 쓰기 때문에 라즈베리파이에서는 장치 경로만
지정하면 되도록 짰지만, 실기 검증은 남은 과제입니다.

```bash
# 라즈베리파이에서 (SPI 활성화 후)
./detect_board --image <보드> --spi /dev/spidev0.0 --spi-speed 1000000
./detect_board --image <보드> --i2c /dev/i2c-1:0x42
```

macOS에서는 spidev/i2c-dev 헤더가 없어 해당 코드가 컴파일에서 빠지고, `--spi`를 주면
"리눅스에서만 지원한다"는 안내가 나옵니다. `--bus-selftest`는 어디서든 동작합니다.
