...
> 대부분의 PCB 결함 검사 프로젝트는 **모델 정확도**까지 측정합니다.
> 이 프로젝트는 **패치 정확도 96%에도 보드에서는 결함의 절반을 놓친다**는 것을 발견한 지점에서 시작해,
> INT8 양자화 · C++ 포팅 · NPU 컴파일까지 **실제 장비에 올라가는 형태**로 만들었습니다.
...
오분류 사례가 더 흥미로운데, **정상 패치를 short(확신도 0.81)로 잘못 본 사례에서 모델은
정상 배선의 잘록한 연결부 양끝을 보고 있었습니다.** confusion matrix에서 세운 가설(배선이
촘촘한 구간은 정상 연결부도 short처럼 보인다)이 그림으로 확인된 셈입니다. 반대로 short를
정상으로 놓친 사례들(확신도 0.41~0.97)은 히트가 결함이 아니라 주변 배선에 퍼져 있어,
미세한 브리지가 정상 패턴에 묻히면 모델이 근거로 잡지 못한다는 것을 보여줍니다.
...
그 기준으로 재보니 recall이 15.6%밖에 안 됐습니다. 원인은 명확했습니다 — 슬라이딩 윈도우
...
TCP 전송까지 파이썬과 같은 기능을 지원하고, 폴더 연속 검사·스레드 파이프라인·SPI/I2C
전송은 C++ 쪽에만 넣었습니다. (직접 작성한 C++ 약 1,530줄)
...
src/serial_bus.*      SPI/I2C 전송, 바이너리 프레임 + CRC-16
```
...
보드 한 장이 아니라 라인 위로 계속 흘러오는 상황을 가정해, 폴더를 통째로 검사하며
처리량(장/초)을 재는 모드를 넣었습니다. 결과는 보드별 한 줄 JSON으로 저장하거나
모니터링 서버로 바로 전송할 수 있습니다.
...
한 장을 처리하는 과정은 `읽기 → 전처리 → 추론`인데, 순차로 돌면 추론하는 동안 다음 장을
읽는 시간이 낭비됩니다. 그래서 생산자 스레드가 읽기·전처리를 맡아 큐에 넣고 메인 스레드가
추론하는 **생산자-소비자 구조**를 만들었습니다. 큐 크기는 2로 제한했는데, 보드 한 장의
입력 텐서가 약 17MB라 제한이 없으면 메모리가 계속 늘어나기 때문입니다.

보드 20장을 두 장비에서 재봤습니다.

| 장비 | 추론 스레드 | 순차 | 파이프라인 | |
|---|---|---|---|---|
| 4코어 리눅스 | 4 (코어 전부) | 3.62초 | 3.90초 | 8% 느림 |
| 4코어 리눅스 | 1 | 7.57초 | 6.83초 | **10% 빠름** |
| 2코어 노트북 | 기본(코어 전부) | 9.42초 | 9.73초 | 3% 느림 |
| 2코어 노트북 | 1 | 13.33초 | 13.02초 | **2% 빠름** |

두 장비에서 방향이 같았습니다. **파이프라인은 남는 코어가 있을 때만 이득**이었습니다. 추론이 이미 코어를 다 쓰고 있으면
생산자 스레드가 CPU를 뺏어가서 손해입니다. 그래서 기본은 순차로 두고 옵션으로 뒀습니다.
코어가 적은 임베디드 장비에서는 추론에 코어를 몰아주는 편이 낫고, 읽기가 느린 장치
(네트워크 스토리지, 느린 카메라)에서는 파이프라인이 유리하다는 판단입니다.

### SPI / I2C로 판정 내보내기

TCP는 관리 PC로 결과를 모을 때 쓰지만, 장비 안에서 판정을 바로 받아야 하는 쪽은 보통
MCU나 PLC이고 그 경로는 **SPI나 I2C 같은 보드 내부 버스**입니다. 대역폭이 낮아 JSON을
그대로 흘리기 어려워서, 길이가 정해진 바이너리 프레임을 설계했습니다.

```
[0xA5 0x5A][버전][판정][길이 2] [보드번호 4][결함수 1][결함 10바이트 x N] [CRC-16 2]
```

결함 하나가 10바이트(종류 1 + 좌표 8 + 확신도 1)라 6건이면 73바이트입니다. 프레임을
만드는 부분과 실제로 내보내는 통로(SPI / I2C / 루프백)를 분리했고, SPI는
`ioctl(SPI_IOC_MESSAGE)`, I2C는 `ioctl(I2C_SLAVE)` + `write()`로 리눅스 드라이버를 직접
씁니다.

SPI·I2C 장치가 없어서 **루프백으로 프레이밍과 오류 검출까지 확인**했습니다.
...
프레임 설계·직렬화·CRC·오류 처리는 검증했고, **실제 버스 파형은 확인하지 못했습니다.**
라즈베리파이에서는 `--spi /dev/spidev0.0`처럼 장치 경로만 주면 되도록 짜뒀지만 실기
검증은 남은 과제입니다.
...
| 박스 정밀화 (IoU 개선) | 논문 기준 recall 15.6% → **75.0%** |
