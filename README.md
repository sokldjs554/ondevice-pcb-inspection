...
| MobileNetV2 | 65개 **100% NPU** (CPU 폴백 0) | 4.889 ms (205 FPS) | 124.4 KB | 95.98% |
...
|---|---|---|---|---|
| MobileNetV2 | 기본 | 97.1% | 95.0% | 77.2% |
| MobileNetV2 | **+ 템플릿 비교** | **97.1%** | **94.9%** | **84.8%** |
...
| **+ 템플릿 비교 + 박스 정밀화** | **75.0%** | **71.9%** | **68.4%** | **71.5%** |
...
recall 97.1%로 충분히 쓸 수 있다고 봅니다. 정확한 박스가 필요하면 검출 모델로 가야 한다는 걸
숫자로 확인한 셈입니다.
...
| 결함 크기 | ~32px | 32~64px | 64~96px | 96px~ |
|---|---|---|---|---|
| 단일 스케일 | 97.6% | 96.0% | 91.8% | 64.3% |
| **멀티스케일** | **99.4%** | **98.1%** | **96.5%** | **92.9%** |

전 구간이 좋아졌고 특히 큰 결함은 64.3%(9/14) → 92.9%(13/14)가 됐습니다.
전체 recall도 95.5% → 98.1%로 올랐습니다.
...
|---|---|---|---|
| 기본 | 97.1% | 95.0% | 77.2% |
| + 템플릿 비교 | 97.1% | 94.9% | **84.8%** |
| + 템플릿 비교 + 멀티스케일 | **99.3%** | **97.3%** | 78.2% |
...
| 시도 | 결과 |
|---|---|
| 템플릿 비교로 오검출 제거 | precision 77.2% → **84.8%** (recall 유지) |
| 박스 정밀화 (IoU 개선) | 논문 기준 recall 14.8% → **75.0%** |
| 멀티스케일 검출 | 96px 초과 결함 recall 64.3% → **92.9%** |
