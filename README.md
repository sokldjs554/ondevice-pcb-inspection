...
| 결함 크기 | ~32px | 32~64px | 64~96px | 96px~ |
|---|---|---|---|---|
| 단일 스케일 | 98.0% | 96.3% | 94.9% | 71.4% |
| **멀티스케일** | **99.5%** | **98.2%** | **98.7%** | **100.0%** |

전 구간이 좋아졌고 특히 큰 결함은 71.4% → 100%가 됐습니다. 다만 검출 수가 늘어나
precision은 떨어집니다. (같은 결함이 두 스케일에서 잡히는 건 IoU 기반 병합으로 정리했습니다)
...
`--bench` 옵션으로 스레드 수별 속도도 재봤습니다 (4코어 x86, 보드 1장, 10회 평균).
